...
public class SolarDataLoader : MonoBehaviour
{
    ...
    // Packed into a single flat float[] for cache locality: [azimuth0, elev0, azimuth1, elev1, ...]
    // A full year is 525,600 minutes -> ~4 MB. Keeping it linear (rather than a Vector2[] or a
    // 2D [day, minute] array) means the export loop's millions of lookups are contiguous reads
    // with no pointer chasing and no per-lookup allocation.
    float[] _data;
    int _loadedYear = -1;
    int _totalMinutes;
    ...
    public bool LoadYear(int year)
    {
        ...
        // Trust the file size over the header: a truncated file would otherwise throw
        // EndOfStreamException partway through the read loop below.
        long availableEntries = (reader.BaseStream.Length - HEADER_BYTES) / 8L;
        if (totalMinutes > availableEntries)
        {
            Debug.LogWarning($"[SolarDataLoader] Header claims {totalMinutes:N0} entries but the file only holds " +
                             $"{availableEntries:N0}. Loading the available entries; sun positions past " +
                             $"day {availableEntries / MINUTES_PER_DAY} will clamp to the last known value.");
            totalMinutes = (int)availableEntries;
        }

        if (totalMinutes <= 0)
        {
            Debug.LogError($"[SolarDataLoader] {path} contains no solar entries.");
            return false;
        }
    ...
    public (float azimuth, float elevation) GetPositionLerped(DateTime localTime, float minuteFraction)
    {
        ...
        // LerpAngle (not Lerp) for azimuth: it takes the shorter arc, so crossing due north
        // interpolates 359 deg -> 1 deg instead of sweeping the long way round the compass.
        // Elevation is monotonic within a minute and needs no wrapping.
        return (
